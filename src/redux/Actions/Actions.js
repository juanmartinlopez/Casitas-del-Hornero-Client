...
export const sendEmail = (email) => {
  return async (dispatch) => {
    try {
      const response = await axios.get(`${URL_BASE}/email/Registro/${email}`);
    } catch (error) {
      swal({
        text: error.response.data.error,
        icon: "warning",
...
export const registerUserFirebase = (user) => {
  return async (dispatch) => {
    try {
      const response = await axios.post(`${URL_BASE}/user`, user);
      GuardarLocalStorage({
...
export const getHotelsAdmin = (id_user) => {
  return async (dispatch) => {
    try {
      const response = await axios.get(
        `${URL_BASE}/hotels/allHotels/${id_user}`
      );
...
export const getAllBookings = (id, rol) => {
  return async (dispatch) => {
    try {
      const response = await axios.get(
        `${URL_BASE}/booking/?id_superadmin=${id}&rol=${rol}`
      );
